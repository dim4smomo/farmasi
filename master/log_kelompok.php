...
<table id="logTable" class="display nowrap" style="width:100%">
    ...
    <tbody>
        ...
        $sql = "
            SELECT 
                l.id_log, l.aksi, l.kelompok_id,
                sl.nama_sumber AS sumber_lama,
                sb.nama_sumber AS sumber_baru,
                l.nama_lama, l.nama_baru,
                l.user, l.created_at
            FROM log_kelompok l
            LEFT JOIN sumber sl ON l.id_sumber_lama = sl.id_sumber
            LEFT JOIN sumber sb ON l.id_sumber_baru = sb.id_sumber
        ...
        <tr>
            ...
            <td><?= htmlspecialchars($row['sumber_lama'] ?? '-') ?></td>
            <td><?= htmlspecialchars($row['sumber_baru'] ?? '-') ?></td>
